...
def _inp(**kwargs):
    defaults = dict(
        project_name="Err", support_tag="FSU-X",
...
def test_no_passing_section_becomes_out_of_scope_status():
    ...
    inp = _inp(
        ...
        fan_units=[FanUnit(fan_type=FanType.CENTRIFUGAL, weight_kg=900.0,
                           operating_weight_kg=950.0,
                           footprint_length_mm=1500.0, footprint_width_mm=1200.0)],
    )
...
def test_blocked_weight_raises_out_of_scope():
    ...
    inp = _inp(fan_units=[FanUnit(fan_type=FanType.CENTRIFUGAL, weight_kg=1400.0,
                                  operating_weight_kg=1500.0,
                                  footprint_length_mm=2000.0, footprint_width_mm=1500.0)],
               confirm_extended_range=True)
