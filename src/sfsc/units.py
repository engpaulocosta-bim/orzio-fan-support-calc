...
def kgf_to_n(kgf: float) -> float:
    return kgf * G_GRAVITY


def kgf_to_kn(kgf: float) -> float:
    return kgf_to_n(kgf) / 1_000.0


# ── Momento ────────────────────────────────────────────────────────────────────
def knm_to_nm(knm: float) -> float:
    return knm * 1_000.0


def nm_to_knm(nm: float) -> float:
    return nm / 1_000.0


def knmm_to_knm(knmm: float) -> float:
    return knmm / 1_000.0


# ── Tensão ─────────────────────────────────────────────────────────────────────
def mpa_to_nmm2(mpa: float) -> float:
    return mpa  # 1 MPa = 1 N/mm²


def nmm2_to_mpa(nmm2: float) -> float:
    return nmm2


def kn_m2_to_mpa(knm2: float) -> float:
    return knm2 / 1_000.0
...
def mm4_to_cm4(mm4: float) -> float:
    return mm4 / 1e4


def cm4_to_m4(cm4: float) -> float:
    return cm4 * 1e-8
